...
Stage 1 should not teach a learner to analyze the whole psychological architecture of an episode.
...
The Gate C designer map contains many distinctions, but a learner should not have to hold them all at once.
...
They are ordered for teaching, not claimed as a universal mental sequence.
...
> **Заметь достаточно, чтобы увидеть: что сейчас происходит и почему это имеет значение для следующего шага.**
...
Recognize at least one currently relevant element such as:
...
- constraint / resource;
- relevant task / responsibility / direction.
...
Without detection, there is no Selection Point.

But more noticing is not automatically better.
...
An external prompt may support LU-1.

Examples:
- reminder;
- questionnaire expected later;
- trainer question;
- environmental cue;
...
- deep introspection;
- finding childhood causes;
- discovering the “true mechanism”;
- mindfulness as endpoint;
- continuous self-observation;
- naming many emotions;
- stopping the old response automatically.
...
> “Я всё заметил и подробно описал.”

but no later action field becomes more open.

## Stage boundary

Stage 1 does not require spontaneous detection under high stress or across all contexts.

Prompted noticing is allowed as supported performance, but it must not be mislabeled independent capacity.

## Relation to next unit

LU-1 creates visibility.

LU-2 tests whether visibility can become an actual interruption window.

## Strongest Red Team attack

A participant can become highly self-conscious, observant and verbally precise while remaining behaviorally unchanged.

**Constraint:** LU-1 is never complete as the practical Stage 1 cycle by itself.
...
> **Не обязательно остановить весь автоматизм. Нужно открыть хотя бы небольшой участок, где старое продолжение перестаёт быть единственным.**

## Purpose

Transform noticing into a usable interruption window.
...
- invoke an if–then response;
- begin a competing action;
- ask for support;
- wait for necessary information.

The learner does not need to return to the original trigger.
...
At least one relevant continuation that would otherwise likely have proceeded automatically is delayed, reduced, stopped, context-shifted, or made non-exclusive long enough for an alternative to become executable.

## False mastery

> “Я сделал паузу.”

but then the exact old continuation resumed with no additional availability or new information.
...
LU-3 determines whether something executable can actually occupy that space.

## Strongest Red Team attack

Training interruption can make people interfere with well-trained useful behavior or become chronically hesitant.

**Constraint:** train selective interruption based on mismatch, not interruption as virtue.
...
> **Выбери не идеальный вариант, а одно реально выполнимое продолжение — и сделай его.**

## Purpose

Convert the opened Selection Point into factual participation.
...
- stop or limit the current action;
- consciously not act;
- wait;
- gather information;
- repair;
...
- prepare the next recurrence;
- revise the move after a new fact.

Then actually realize it.

## Prepared alternative principle

Where an old route is strong, the replacement may need to be prepared before the cue arrives.
...
Interruption without an available replacement often leaves the old route as the easiest route.
...
- maximizing options;
- making the “best life decision”;
- proving autonomy through arbitrary action;
- choosing what the trainer prefers;
- mentally deciding without execution;
- obtaining the desired outcome.

## Observable-expression candidate

The participant separately states:

- what continuation was selected;
- what was actually realized;
- whether realization was full, partial, conscious non-action, revised after new fact, not executed, or unknown.
...
without factual realization.

## Scaffold boundary

A prompt may retrieve a preselected alternative.

If the action occurred because the prompt was present, that support remains part of the evidence.

Prompt-supported execution is real execution, but not proof of independent retrieval.

## Stage boundary

Stage 1 requires one available realized continuation.

It does not require generalization across domains or stable execution under severe load.
...
The learner may simply obey an external script.

**Constraint:** the participant must own the meaningful continuation; scaffolding may cue/retrieve but not silently choose the substantive move.
...
- explain the origin of automaticity;
- identify psychological mechanisms correctly;
- eliminate impulses;
- eliminate automaticity;
- be constantly aware;
- remember the full designer map;
